...
class PetstoreDb {
   ...
   *
   * Also validates that a current version of
  ...
  /**
   * Find pets by status. Multiple statuses can be provided with comma-separated strings.
  ...
  /**
   * Find pets by [[Tag]]. Multiple tags can be provided with comma-separated strings.
  ...
  /**
   * Updates a [[Pet]] in the store (name and status only).
  ...
  def getInventory: Future[Inventory] = Future.value(
    pets.synchronized {
      ...
      val available: Int = stock(Status.Available)
      val pending: Int = stock(Status.Pending)
      val adopted: Int = stock(Status.Adopted)
  ...
  /**
   * Get [[User]] by username, assume all usernames are unique.
  ...
  /**
   * Update a [[User]]. Note that usernames cannot be changed because they are the unique identifiers by which the
